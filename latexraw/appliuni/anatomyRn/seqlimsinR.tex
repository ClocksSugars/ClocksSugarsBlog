...
\begin{label example}[$1/n^2$ goes to $0$ as $n$ goes to $\infty$]{IntroInverseNSquaredLimit}{mo}
...
\begin{label example}[$2^{-n}$ goes to $0$ as $n$ goes to $\infty$]{m}{ConvergenceofTwotothenInR}
Let us repeat the steps we did prior but more streamlined this time. In this case we notice we are studying a strictly monotonic decreasing function $n \mapsto 2^{-n}$ which is always $2^{-n} > 0$, so immediately a condition $|2^{-n} - 0| \le \varepsilon$ can be read instead as $2^{-n} \le \varepsilon$. This may inspire us to take an inverse, and indeed we can notice that if we could choose $\varepsilon = 2^{-N}$ then $n \ge N$ would imply $2^{-n} \le \varepsilon$ as we desire, so we want to propose something like $N = - \log_2 \varepsilon$. The negative sign here is not to make $N$ negative (indeed we do not want this) but instead to make it positive, since a $\varepsilon < 1$ will have $\log_2 \varepsilon \le 0$, a property of logarithms. The problem we actually have is the same as in the previous example, that we have no guarentee that $- \log_2 \varepsilon$ is a whole number, so we will want to round it up as $N = \lceil - \log_2 \varepsilon \rceil$.

With this $N$ proposed, we can now prove the limit. First observe that \begin{gather*}
N = \lceil - \log_2 \varepsilon \rceil \ge - \log_2 \varepsilon
\end{gather*}
so we may apply the monotonic decreasing function to the inequality, $x \mapsto -x$, obtaining \begin{gather*}
- N \le \log_2 \varepsilon
\end{gather*}
with the appropriate sign flip, followed by $x \mapsto 2^x$ to undo the logarithm (which is a monotonic increasing function), \begin{gather*}
2^{- N} \le \varepsilon.
\end{gather*}
If we repeat these operations on $n \ge N$ we obtain \begin{gather*}
-n \le -N \\
2^{-n} \le 2^{-N}
\end{gather*}
providing the transitive chain \begin{gather*}
|2^{-n} - 0| = 2^{-n} \le 2^{-N} \le \varepsilon \\
|2^{-n} - 0| \le \varepsilon
\end{gather*}
...
\figuresvgwithcaption{./basicconvergence}{A visualisation of a converging sequence beside the real number line, with the sequence points represented by dots converging on the limit $L$. We represent the region of convergence corresponding to a shrinking $\varepsilon$ as the sequence continues with red braces. Eventually we are able to say that \squote{the rest of the sequence} is contained in smaller and smaller regions.}
