...
Then $a+\hat{b}$ would be an upper bound for $(a_n)_{n\in \nats}$ if not for the fact that some $a_n$ with $n < N$ might be greater than this upper bound; to fix this, we will define an upper bound for all $a_n$ before $n < N$, and we do this first by defining the set of points before $n < N$, $A_N = \{a_m \mid n < N\}$. With this set, we can take its maximum $\max(A_n)$ and either $\max(A_N)$ or $a+\hat{b}$ will be the upper bound for the entire sequence, so let us define \begin{gather*}
