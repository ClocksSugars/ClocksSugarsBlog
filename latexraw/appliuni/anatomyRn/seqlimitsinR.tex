...
In fact it is true that the limit is homomorphic in a sense; our notion of division must surely be weakened since in order to preserve the structure of a division, the divisor must \emph{never} be zero. But this is still enough to satisfy the requirements for a \emph{partially ordered ring homomorphism}, a map that preserves the structures of addition, subtraction, multiplication and ordering, but not division. Strictly speaking it preserves even more structure than that, but that is certainly the neatest existing class of homomorphism we can put the limit in.

The clear implication of this, if you wish to dwell on it, is that there is something \emph{number-like} about sequences of real numbers, much in the way we expect real numbers to function as numbers
