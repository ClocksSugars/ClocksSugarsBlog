...
Below I'll include a few examples so you get an idea of how a limit on a function is shown. In all likelihood, one of the examples here will later be important for some other proof, so it's likely to expand.

\begin{example}[Showing convergence of $(x-1)/(x-1)$]{m}
Let's continue our prior example first, although it will quickly become apparent that this is one of the easiest kinds of limits to show. 
\end{example}
