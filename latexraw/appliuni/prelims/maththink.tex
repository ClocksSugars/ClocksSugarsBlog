...
As this will be our first exploration of a mathematical concept from its axioms, we should discuss a bit about how this will work. Our notion of propositional logic will carry over quite strongly from the previous section, and our axioms will define a collection of propositions asserted true about sets. These will be the requirements, the rules, that restrict down the could-be-anythings of unspecified symbols down to a collection of certainly-are-somethings from which we can make deductions. Importantly, a set of axioms is often far from a literate series of statements about what a thing is or how it should be interpretted; that information is certainly \emph{encoded}, but it is as a deduction from what is said or not said in the axiomatic statement. (It is often the case that axioms will try to be relatively minimal, making as \emph{weak} a statement as possible so that properties that are desireable can be encountered or deduced rather than enforced).

As such, our reading of the Zermelo-Fraenkel axioms will start from the axioms themselves, how one can read or interpret each axiom, and some basic consequences. This is the process we must eventually go through habitualy when encountering a new set of axioms. In this text I will generally try to preempt (or post-empt) such axiomatic definitions with a rough intuition for what the axioms are trying to describe, as I did earlier, although one should not expect such gentleness from most mathematics texts.
...
Consider the following example. If we interpret the construction we see in the axiom of infinity to be the natural numbers, then we can apply the axiom of extensionality and say that our zero has $\mathbf{Z} = \mathbf{Z}$ since $\mathbf{Z} = \emptyset$ and the axiom of extensionality says $\emptyset = \emptyset$. We can subsequently do this for every nested-set integer implied by the axiom of infinity since, while we may not have a notion of equality from elsewhere, we do have one for sets and objects which are merely sets in disguise. But the moment we declare that the natural numbers we use are some other object which is not itself a set but are contained in a set via the axiom of replacement (e.g. say we define a function which takes axiom-of-infinity numbers to inductive natural numbers and use the axiom of replacement to define this as a \squote{set of natural numbers}) we have to import our own notion of equality.

While the sets we declare as subjects of study (i.e. the real numbers, algebras over fields, groups, etc.) will generally come equipped with their own notion of equality, or at least a 
