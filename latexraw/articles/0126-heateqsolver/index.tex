...
Now remember way back at the beginning when we requested our adapter, and there was that one argument \texttt{compatible\_surface} which we ignored? We'll need that now, as we actually have a surface we'll want to draw to. If you're doing this on the desktop, you'll want to create your surface in a different way outlined \hyperlink{https://sotrh.github.io/learn-wgpu/beginner/tutorial2-surface/}{here}, probably using the \texttt{winit} crate.
...
The job of our fragment shader is rather opaque; we have GPU steps for using the sampler and view to do whatever texture sampling WGPU finds appropriate. Our vertex shader we'll be significantly more hands on with. Since we want to simply draw our texture to the screen, we can achieve this with two triangles forming a square over the view. To specify those triangles, we'll need to give coordinates, but we might have to give those coordinates multiple times since these two triangles forming a square will share vertices. It's better practice when we're certain we want the edges of the triangles to line up to simply reuse the points together with an \emph{index buffer} which we'll use to say which triangles have which vertices as their points.

It'll also be important to think about which vertices we specify for the triangle in which order, since our triangles have a front face and a back face. If we view a triangle through its back face, it will be culled to the viewport, so picking the order of vertices correctly is paramount to actually getting to see the thing. My implementation is inspired heavily by \hyperlink{https://sotrh.github.io/learn-wgpu/beginner/tutorial5-textures/}{sotrh's tutorial} which gives some details that I haven't, including how coordinates work for textures.
...
\begin{my proof}
[Render pipeline shader code and render function is minimized here]{m}
The following is basically just \hyperlink{https://sotrh.github.io/learn-wgpu/beginner/tutorial5-textures/}{sotrh's code}, but if you have any interest in reproducing what I made, you'll need this.
\begin{lstlisting}[language=Rust]
...
No matter what you do, you'll need a way to make this state accessible to javascript. One way to do this is to make the state itself into a javascript object, which can then have its methods called over there. The way to do something like that is outlined \hyperlink{https://wasm-bindgen.github.io/wasm-bindgen/reference/types/exported-rust-types.html}{here}. What I elected to do to keep as much inside rust as possible is to maintain a persisting state variable. This can be achieved with the \texttt{thread\_local} macro and the \texttt{RefCell} type.
...
We're almost ready to actually call some functions from javascript, but first we need to discuss the initialization. This is not as simple as our GPU initialization since WASM needs a distinct entrypoint where it can also set up everything WASM itself needs to run in javascript. That entry point must be marked \texttt{\#[wasm\_bindgen(start)]}. This is also a good time to initialize the logging tools that will let us write to the browser console if we want to do some debugging.
...
We need to call \texttt{init} before we use anything from WASM or else it will fail. As I mentioned earlier, we can use both graphics and export the buffer which I do here in \texttt{get\_total\_energy\_in\_one} when we sum the temperature across all grid cells so we can check if our simulation is conserving energy, hence we make use of receiver stored in \texttt{INTERNAL\_MESSAGE}.
...
The rest of the features I implemented, including the ability to implement an initial configuration from a CSV, amount to details you can find on my \hyperlink{https://github.com/ClocksSugars/pet_webgpusolver/tree/main}{github repo for this project}. As you can imagine it's not that hard to parse a CSV, and all other functions are merely derivative of what I've described above. There is the matter of how you run this on desktop, in which case \hyperlink{https://sotrh.github.io/learn-wgpu/beginner/tutorial1-window/\#boring-i-know}{sotrh's tutorial} using \texttt{winit} applies more; my code also does both, but doesn't implement the full suite of features made available on the web for the desktop app at this stage.

To compile the project, you'll need to install \hyperlink{https://github.com/drager/wasm-pack}{wasm-pack} and run the command \texttt{wasm-pack build --target web}.


\subsection{Numerics Considerations}

Now that we know how to build the simulation, we need to ask some basic questions about when it will give us a correct answer and how correct that answer will be. Those two basic considerations correspond to the truncation error and the stability analysis.

We'll start with the stability analysis. The rational for how this works can be a bit conceptually deceptive but it comes down to two fundemental observations.

First, our method revolves around allowing pixels on our grid to conduct heat into adjacent pixels. If we recall the Green's function solution, \begin{gather*}
T(x,t) = \frac{1}{\sqrt{4 \pi \kappa t}} \int_{-\infty}^{\infty} \exp\left( - \frac{(x - \tau)^2}{4 \kappa t}\right) g(\tau) d\tau
\end{gather*}
we'll see that there's a gaussian, which in a sense means that for our simulation to be perfectly accurate we need a way to make every pixel affect every other pixel simultaneously, even if only a little. In practice, since the exponential falls off, well, exponentially, each part of our grid doesn't meaningfully affect evvery other one. But in principle we must now pose the question if it is possible for the true solution to meaningfully spread out, more than what might merely be a rounding error, faster than our adjacent-effector time step allows? If that were to happen, we'd have no reason to think that our model would predict something sensible. In fact the simulation might try to guess that heat should flow more than the difference between temperatures, meaning that we'd see net heat-difference creation; this would be like watching a system's entropy go down in a sense. No good.

So how are we to know when our timestep is small enough? The solution comes to us through history; we must remember that fourier analysis is famed early on for its contributions to the study of the heat equation, and so our method of attack will be to study our solutions $T(x,y,t)$ in terms of a wave basis of functions $T(x,y,t) = E(t) e^{i k_x x + i k_y y}$, composed of a magnitude term $E(t)$ and a complex wave. We're using a complex wave but that doesn't mean we're necessarily thinking about some notion of complex temperature, just that we're decomposing our functions that way, with $k_x$ and $k_y$ standing for the frequency of the wave in different axes. Since the heat equation is linear, any solution of this form will have a corresponding $E(t) e^(-ik_x x - i k_y y)$ solution which will add to make $E(t) cos(k_x x + k_y y)$ for example.
...
\end{align*}
we obtain an expression for the ratio of $E(t)$ growth. Since this was the magnitude over time, this ratio tells us whether or not the magnitude of the wave component is growing, and so any unstable solution will need that to be positive in order to produce the weird heat propogation effects of an unstable simulation. One might infer that we can simply demand that this growth rate is less than one, i.e. that the magnitude of plane waves in the simulation only declines, and in turns out this works great in this case. So let's assert \begin{gather*}
...
\hyperlink{https://clockssugars.blog/appliuni/anatomyRn/realnumsaxioms.html\#RAbsoluteValueProperties}{Elsewhere on this site} I've explained why $|x|\leq y$ implies $-y \leq x \leq y$, so we'll apply that here to get two inequalities. \begin{gather*}
...
\begin{my proof}[Theorem: RK2 Does not provide Stability Improvements]{}
No it can't.
\end{my proof}
\begin{my proof}{m}
...
\end{pmatrix} \ge 0
\end{align*}
We have a term of order $O(\kappa \Delta t/ \Delta x^2) + O(\kappa \Delta t/ \Delta y^2)$, which is the same term as in the non-RK2 case, and all other terms have the corresponding coefficient squared or some variation of that involving swapping $\Delta x^2$ or $\Delta y^2$. So already, we expect our bound to be \emph{close} to the normal stability bound because the standard term dominates. Moreover, many of the sine arguments are related, which is important since $\sin^2$ has all of its peaks at odd number multiples of $\pi/2$ and zeros for even multiples. So if we were to maximise the standard non-RK2 sine squared terms, we'd zero out many of the other ones in the following way: \begin{align*}
...
\end{gather*}
If we want to get a bound with $\Delta t$ on one side of this, we'll need to solve this as a quadratic in $A$. We can apply the quadratic equation on both inequalities that form this transitive chain, but if we do this for the $2 \ge $ side we get complex solutions. Proceeding with the other inequality, the $\ge 0$ branch, \begin{gather*}
A^2 \left( -8 B^2 -8C^2 -16 BC \right) + A(4B + 4C) \ge 0 \\
A \Big( A \left( -8 B^2 -8 C^2 -16 BC \right) + 4B + 4C \Big) \ge 0
\end{gather*}
The solutions, and thus the terminals of the range $A$ must remain inside, are $\kappa \Delta t = 0$ (meaning things are unstable if $\kappa \Delta t$ is negative) and \begin{align*}
\kappa \Delta t = \frac{4B + 4C}{8B^2 + 8 C^2 + 16 BC} =&
\frac{4/\Delta x^2 + 4/\Delta y^2}{8/\Delta x^2  + 8/\Delta y^2 + 16/ \Delta x^2 \Delta y^2} \\
=& \frac{1/\Delta x^2 + 1/\Delta y^2}{ (2\Delta x^2 + 2 \Delta y^2 + 4)/ \Delta x^2 \Delta y^2}
\end{align*}

\end{my proof}

%\frac{\delta^2 T^{i,j}_k}{\delta x^2} +\frac{\kappa \Delta t^2}{2} \nabla_\square^2 \frac{\delta^2 T^{i,j}_k}{\delta y^2} 
...
\end{gather*}
which completely ignores those other terms in the Taylor series. If our $\Delta x$ (or our $\Delta t$ for that matter) is small enough then ignoring these terms may merely be a rounding error, but this error is sure to add up over time.

We can use this as a method for comparing numerical techniques; essentially we ask the question, for a given technique, how much does our error reduction decrease when we make our calculation more expensive.
...
Recall our iteration method again: \begin{align*}
\frac{\partial^2 T}{\partial x^2}(x,t) &= \frac{T(x + \Delta x,y, t) - 2 T(x,y,t) + T(x - \Delta x,y, t) }{\Delta x^2} \\
\nabla^2 T(x,y,t) &= \frac{\partial^2 T}{\partial x^2}(x,y,t) + \frac{\partial^2 T}{\partial y^2}(x,y,t) \\
T(x,y, t + \Delta t/2) &=  T(x,y,t) + \frac{\Delta t}{2} \cdot \kappa \nabla^2 T(x,y,t) \\
T(x,y, t + \Delta t) &= T(x,y,t) + \Delta t \cdot \kappa \nabla^2 T(x,y,t + \Delta t /2)
\end{align*}
We must recall that the latter two of these equations were derived from the following assumptions: \begin{align*}
\frac{T(x,y, t + \Delta t/2) - T(x,y,t)}{\Delta t / 2} &=   \kappa \nabla^2 T(x,y,t) \\
\frac{T(x,y, t + \Delta t) - T(x,y,t)}{\Delta t} &=   \kappa \nabla^2 T(x,y,t + \Delta t / 2)
\end{align*}
