...
We \emph{can} substitute all of these equations into one, move terms to one side and set that to the truncation error, but doing this results in a \emph{very} long formula. Let's separate that error into three parts $\text{TE}_x$, $\text{TE}_y$ and $\text{TE}_t$. \begin{gather*}
\text{TE} = \text{TE}_t + \Delta_t(\text{TE}_x + \text{TE}_y)
\end{gather*}

It is also insightful to notice that the iteration step of our method, although divided into two parts, can be written in just one: \begin{gather*}
T(x,y,t + \Delta t) = T(x,y,t) + \kappa \Delta t \big( T(x,y,t) + \frac{\Delta t}{2} \cdot \nabla^2 T(x,y,t) \big)
\end{gather*}

Another thing we need to look out for is whether or not our method converges at all; consider the possibility that our simulated system ends up in some state that is able to cause a run-away feedback loop
