...
This mode of reasoning also applies immediately to certain things about numbers. For instance, the idea that $m \equiv n$ implies $m + 1 \equiv n + 1$ is immediate in this manner: a function \begin{gather*}
...
\end{gather*}
with witness given trivially by $\mathtt{refl} \hspace{0.3em} (\mathbf{S}(n))$.

But our system can do much more than just make things we knew intuitively to be true much more burdensome to prove, it can make things which are often difficult for students to comprehend equally as concrete as those we knew to be obvious already. The famous \squote{proof by induction} is brought into crystal clarity by our technique, and in fact \emph{deduced}.

%In this mode of reasoning, we see that the idea $a = b$ implies $a + 1 = b + 1$ is not even obvious either; where is our function $\forall (a,b) \colon A \times A, (a \equiv b) \Rightarrow (\mathbf{S}(a) \equiv \mathbf{S}(b))$? In fact, to show this we need \squote{proof by induction} in its fully formalized form, and indeed we are able to \emph{prove} that proof by induction is even possible.
...
In practice, we will rarely use this kind of reasoning in mathematics, since, while a useful tool for being absolutely certain no logical error has been made, we typically try to adjust our intuitions so that we can reason such steps automatically. That is not to say, in this moment at least, that the rigidity we expect of pure mathematics is any weaker, but that statements such as $a = b \Rightarrow a + c = b + c$ is one we train our intuitions on so that we know automatically how to make \emph{algebraic} deductions.
...
\begin{my proof}
\begin{itemize}
\item Let us proceed as if $P,Q$ were normal types. Then we read $\neg P \vee \neg Q = (P \to \bot) + (Q \to \bot)$ and $\neg (P \wedge Q) = P \times Q \to \bot$. Together then, we must find a witness function of the type \begin{align*}
...
Moreover, this more metaphysical interest means that there are circumstances even when even mathematicians, looking at a statement that agrees with their metaphysical understanding of an object, will be tempted to say \squote{this seems true enough}. Such is famously the case with the \emph{axiom of choice}.
...
Let $P,Q \colon \mathtt{Type}$ be \emph{decidable} types, that is, we have witnesses of $P \vee \neg P$ and $Q \vee \neg Q$, and let $R \colon A \to \mathtt{Type}$ be a family of decidable types. Then we have the following: \begin{itemize}
\item Proof by contradiction when a proposition is decidable, i.e. \emph{Reductio ad absurdum} \begin{gather*}
...
\item $\neg \big(\forall (a : A), R(a) \big) \Leftrightarrow \exists (a : A), \neg R(a)$ i.e. \squote{not all if and only if there exists a counter example}
...
\item Proof by contrapositive \begin{gather*}
(P \Rightarrow Q) \Leftrightarrow (\neg Q \Rightarrow \neg P)
\end{gather*}
\item Negation of implication \begin{gather*}
\neg (P \Rightarrow Q) \Leftrightarrow P \wedge \neg Q
\end{gather*}
\end{itemize}
\end{label theorem}

Since this proof is quite long and largely an application of previously applied techniques, we by default minimize it in the web version. Please click it to expand.

\begin{my proof}
We have already shown some parts of these in theorem \ref{thm:IncompleteNegationRules}, so we only need to show the remaining parts. \begin{itemize}
...
f \colon \forall (P : \mathtt{Type}), \bigg( & P \vee \neg P \to \forall (Q : \mathtt{Type}), \Big( Q \vee \neg Q \to  \\
& \big((P \times Q) \to \bot \big) \to \big( (P \to \bot) + (Q \to \bot) \big) \Big) \bigg)
\end{align*}
...
f \hspace{0.2em} P \hspace{0.2em} (l \hspace{0.2em} p) \hspace{0.2em} Q \hspace{0.2em} (l \hspace{0.2em} q) \hspace{0.2em} h = k \hspace{0.2em} \big(h \hspace{0.2em} (p,q) \big) \\
f \hspace{0.2em} P \hspace{0.2em} (l \hspace{0.2em} p) \hspace{0.2em} Q \hspace{0.2em} (r \hspace{0.2em} g_q) \hspace{0.2em} h = r \hspace{0.2em} g_q \\
f \hspace{0.2em} P \hspace{0.2em} (r \hspace{0.2em} g_p) \hspace{0.2em} Q \hspace{0.2em} (l \hspace{0.2em} q) \hspace{0.2em} h = l \hspace{0.2em} g_p \\
f \hspace{0.2em} P \hspace{0.2em} (r \hspace{0.2em} g_p) \hspace{0.2em} Q \hspace{0.2em} (r \hspace{0.2em} g_q) \hspace{0.2em} h = l \hspace{0.2em} g_p
\end{gather*}
...
\item We may reuse a lot of the previous reasoning. We already showed $\neg P \wedge \neg Q \Rightarrow \neg (P \vee Q)$ so now we must show $\neg (P \vee Q) \Rightarrow \neg P \wedge \neg Q$, and to do so we must once again pattern match on our decidability types. Let $f$ be our function as before with the same first four arguments, the fifth now being of type $h \colon (P + Q) \to \bot$ and our desired output type being $\neg P \wedge \neg Q$. This time, let us define $k_*$ denote our trivial function wild-card. That is, if we can prove false, let us say that anything desired is proven instead of defining a trivial function for each type. \begin{gather*}
f \hspace{0.2em} P \hspace{0.2em} (l \hspace{0.2em} p) \hspace{0.2em} Q \hspace{0.2em} (l \hspace{0.2em} q) \hspace{0.2em} h = k_* \hspace{0.2em} \big(h \hspace{0.2em} (l \hspace{0.2em} p) \big) \\
f \hspace{0.2em} P \hspace{0.2em} (l \hspace{0.2em} p) \hspace{0.2em} Q \hspace{0.2em} (r \hspace{0.2em} g_q) \hspace{0.2em} h = k_* \hspace{0.2em} \big(h \hspace{0.2em} (l \hspace{0.2em} p) \big) \\
f \hspace{0.2em} P \hspace{0.2em} (r \hspace{0.2em} g_p) \hspace{0.2em} Q \hspace{0.2em} (l \hspace{0.2em} q) \hspace{0.2em} h = k_* \hspace{0.2em} \big(h \hspace{0.2em} (r \hspace{0.2em} q) \big) \\
f \hspace{0.2em} P \hspace{0.2em} (r \hspace{0.2em} g_p) \hspace{0.2em} Q \hspace{0.2em} (r \hspace{0.2em} g_q) \hspace{0.2em} h = (g_p,g_q)
\end{gather*}
\end{itemize}
\end{my proof}
