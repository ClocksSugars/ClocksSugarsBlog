...
Defining what a set is then is relatively difficult without quickly sounding silly, and indeed a few books I looked at do walk directly into the notion of a \squote{set of people on earth} or a \squote{set of numbers}. Instead, we'll just put all of the Zermelo–Fraenkel set theory axioms here and then we can discuss what they mean, and what they tell us about sets.
...
A \times B = \left\{ x \in \powerset\big( \powerset( A\cup B )\big)\mid \exists a\exists b, (a \in A) \wedge (b \in B) \wedge (t = (a,b))\right\}
\end{gather*}
...
This is the standard construction of integers in mathematics. If by counting numbers we mean numbers that are strictly greater than or equal to one (this is the standard in mathematics as well, not starting from zero) then we invent negative numbers in this way. From the perspective of already having negative numbers, one easily manipulates this equation to see that it is read $a - b = x - y$, but this is invalid on natural numbers. But if we speak only of positive numbers, then we avoid the problems of asking what $a-b$ is when $b > a$ and instead speak of larger numbers rather than smaller ones.
...
In our case however, one might be concerned about such an aggressive tactic as erasing identifying information out of an object. Surely we are not saying that all pairs of natural numbers $(m,n)$ are integers, right? Integers are supposed to be at the same standing as natural numbers, or real numbers. Enabling different operations of course, but still distinct kinds of objects themselves.

This is in part our expectations about types creeping in, but these are not exactly bad expectations to have. The set theorists, prior to a similar popularity of type theory as we enjoy today, saw fit to try to formalize what we have done above. Thus we have the formal notion of a partition, equivalence relation, and equivalence class, as well as the less formal notion of a class.

To explore this, we will need to formally investigate some terms that were mentioned briefly in the previous section.

\begin{definition}[Equivalence Relation]
An equivalence relation can be considered to be a set of pair-sets $A$. When a pair $\{a,b\} \in A$ then we write $a \sim b$ and either say that $a$ is \emph{similar} to $b$ or that $a$ is \emph{equivalent} to $b$ under $\sim$. Let $B = \cup A$, the set of all items that $\sim$ relates to one another. Then in order to be an equivalence relation, $\sim$ must satisfy the following: \begin{itemize}
\item (Reflexivity) For all $a \in B$, we have $a \sim a$ since a thing must be equal to itself
\item (Symmetry) For all $a,b \in B$, if we have $a \sim b$ then we have $b \sim a$ since equality (or similarity) is not an ordered property; this is automatically satisfied if we think of $\sim$ as defined by a set of pair-sets rather than a set of ordered pairs.
